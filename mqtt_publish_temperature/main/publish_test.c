...
static const char *MY_TOPIC = "testTopic";
static const char *MY_TCP_URI = "mqtt://192.168.0.83/:1883";

static const int MY_DEVICE_ID = 10;
...
static void mqtt_app_start(void)
{
    ...
    const esp_mqtt_client_config_t mqtt_cfg = {
        .username = "admin",    //  karl.kwon
        .password = "admin",    //  karl.kwon
...
//        free(actual_data);
        ...
        // if (0 == strcmp(transport, "tcp")) {
            ...
            // esp_mqtt_client_set_uri(mqtt_client, MY_TCP_URI);
        ...
        ds18x20_measure_and_read_multi(SENSOR_GPIO, addrs, 1, temps);
        ...
        temp_c *= 10.0;
        actual_size = sprintf(expected_data, "{\"ID\":%d,\"T\":%d}\n", MY_DEVICE_ID, (int)temp_c);
        printf("%s\n", expected_data);

        int msg_id = esp_mqtt_client_publish(mqtt_client, MY_TOPIC, expected_data, actual_size, qos_test, 0);
        ...
        for (int i = 0; i < expected_published; i++) {
            ...
            int msg_id = esp_mqtt_client_publish(mqtt_client, MY_TOPIC, expected_data, expected_size, qos_test, 0);
            ESP_LOGI(TAG, "[%d] Publishing...", msg_id);
            printf("\n");
        }*/
    }
